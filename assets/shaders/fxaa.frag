...
void main() {
	...
	vec3 rgbNW = texture2D(source, uv + (vec2(-1.0, -1.0) / frameBufSize));
	vec3 rgbNE = texture2D(source, uv + (vec2(1.0, -1.0) / frameBufSize));
	vec3 rgbSW = texture2D(source, uv + (vec2(-1.0, 1.0) / frameBufSize));
	vec3 rgbSE = texture2D(source, uv + (vec2(1.0, 1.0) / frameBufSize));
	vec3 rgbM = texture2D(source, uv);
